...
${lang === 'es' ? 'Hola Yeison, estoy interesado en discutir este proyecto.' : 'Hello Yeison, I am interested in discussing this project.'}
    `.trim();
  ...
  const handleContactMethodSelect = (method: string, methodLabel: string) => {
    ...
    if (method === "WhatsApp") {
      ...
      setTimeout(() => {
        window.open(generateWhatsAppLink(updatedData), "_blank");
      }, 1500);
    } else {
        ...
        return (
          <div className="flex flex-col gap-2 mt-4">
            <OptionButton
              ...
              onClick={() =>
                handleOptionClick(t.options.build, "qualification", {
                  intent: "build",
                })
              }
            />
            <OptionButton
              ...
              label={t.options.ai}
              onClick={() =>
                handleOptionClick(t.options.ai, "qualification", { intent: "ai" })
              }
            />
            <OptionButton
              ...
              onClick={() =>
                handleOptionClick(t.options.hire, "details", { intent: "hire" })
              }
            />
          </div>
        );
      ...
      case "confirmation":
        return (
          <div className="mt-6 flex flex-col gap-3">
            <a
              href={`mailto:${db[lang].personalInfo.social.email}`}
            ...
            >
              {t.options.contactNow} <Mail size={16} />
            </a>
            <button
      ...
      <AnimatePresence>
        {!isOpen && showPrompt && isBubbleVisible && (
          <motion.div
            ...
            className="fixed bottom-28 right-8 z-[100] max-w-[200px]"
          >
            <div className="bg-white border-2 border-black px-5 py-3 rounded-2xl shadow-[4px_4px_0px_rgba(0,0,0,1)] relative">
              <div className="relative z-10 flex items-center gap-3 text-xs font-bold tracking-wide text-black">
      ...
      <AnimatePresence>
        {!isOpen && showPrompt && (
          <motion.div
            ...
            className="fixed bottom-8 right-8 bg-[#2e1065] border border-white/20 p-1.5 pr-6 rounded-full shadow-[0_8px_30px_rgba(46,16,101,0.6)] cursor-pointer z-[100] flex items-center gap-4 group transition-all overflow-hidden"
          >
